...
log('Start creating app db and app db admin')
...
if (appDbUsers.users.find(it => it.user === appDbUserName)) {
  ...
  *!/
  log(`App db user was created successfully: ${appDbUserName}`)
}
*/


{
  const mongoAppBackendCertClientUserName = process.env.MONGO_APP_DB_USERNAME
  const appDbName = process.env.MONGO_APP_DB_DATABASE
  ...
  if (users.users.find(it => it.user === mongoAppBackendCertClientUserName)) {
    log(`User mongoBackendClientCertRootUser already exists: ${mongoAppBackendCertClientUserName}`)
  } else {
    externalDb.runCommand({
      createUser: mongoAppBackendCertClientUserName,
      roles: [{ role: 'readWrite', db: appDbName }],
    })
    log(`User mongoBackendClientCertRootUser was created successfully: ${mongoAppBackendCertClientUserName}`)
  }
}
...
function log(str) { console.log(`INFO>>>> ${str} { file: 04-app-create-db-and-admin.js }`) }